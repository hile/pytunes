...
from musa.playlist import m3uPlaylist
from pytunes.client import iTunes,Track

SKIP_PLAYLISTS = [ 'Music', 'Movies', 'TV Shows', 'Purchased']
...
class iTunesPlaylist(object):
    ...
    def __getitem__(self,item):
        try:
            item = int(item)+1
            return Track(self.itunes.get(self.playlist.file_tracks[item]),self.itunes)
        except ValueError:
            self.__update_len()
